...
- `app/admin/*` — admin pages (dashboard, qr, quests, achievements, committee, groups, points, announcements, clubs, plus `guide`, a static onboarding page describing every admin tab). `/admin/npc` was removed and replaced by `/admin/qr`; `/admin/quests/onboarding` was replaced by `/admin/guide`.
...
**Quest flow** (the second QR mechanic, added 2026-07-21): a `Quest` is a mission that is *not* tied to a person — one printed code is scanned by every student, and `QuestProgress`'s unique `(studentId, questId)` is the duplicate guard (the same shape as `ScanLog`). Admin creates the quest at `/admin/quests`, generates its QR via `/api/quests/qr` (signs `{ kind: 'quest', questId }`, 120-day expiry — quest posters get printed weeks ahead), prints it, then activates the quest. Scanning calls the `complete_quest` RPC, which inserts the progress row, adds points/xp, and inserts a `StudentAchievement` when `Quest.achievementId` is set. **`/api/qr/scan` is one endpoint for both mechanics**: it verifies the JWT and dispatches on the presence of `questId` to `lib/scan/quest.ts`, else `lib/scan/npc.ts` (dispatching on `questId` rather than requiring `kind` keeps fun-fact QRs printed before quests existed working). Both handlers re-check `isActive` and `qrToken === token` because neither RPC does — regenerating a QR only overwrites the stored token, leaving the old JWT signed and unexpired. `Achievement`s are created at `/admin/achievements` with uploaded badge art and are **only** obtainable by completing a linked quest, so one with no quest pointing at it is unearnable (the admin table flags this). Students see quests at `/quests` (fully visible before completion — it's a mission board) and badges at `/profile`.
