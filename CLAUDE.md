...
`scripts/` holds three re-runnable asset scripts, none wired into an npm script: `extract-group-logos.mjs` slices the 15 group logos out of `REFERENCE/group-logo.svg` into `public/images/group/`, `downscale-pixel-art.mjs` returns upscaled pixel art in `public/images/` to its native grid (see "Pixel avatars" below), and `shrink-storage-images.mjs` re-applies the `lib/storage.ts` resize policy to objects already in Supabase Storage (see "Image uploads").
...
- **`Announcement` has no `type` column** — not live, not in `schema.sql` — but `adjustPoints` in `app/admin/actions.ts` inserts one with `type: 'points'` to log every admin point change. supabase-js returns that error rather than throwing and the call site doesn't check it, so **the point-announcement feature currently writes nothing at all.** Adding `alter table "Announcement" add column if not exists "type" text` is what turns it on; note `Announcement` is read only by `/admin/dashboard` and `/admin/announcements`, so this is an admin-side log, not something students see.
...
- `app/admin/*` — `dashboard`, `committee`, `present`, `quests`, `achievements`, `groups`, `points`, `announcements`, `timeline`, `lunch` (+ `lunch/menu`, `lunch/recap`, `lunch/settings`), `clubs`, `guide` (a static page describing every admin tab). The left rail is driven by `components/admin/ADMIN_NAV.ts`; `AdminShell` highlights via `pathname.startsWith(href)`, so entry order matters — which is why lunch gets **one** rail entry and its sub-pages are reached through the tab strip in `app/admin/lunch/LunchTabs.tsx` (a second entry would prefix-collide). Legacy paths kept as redirects: `/admin/qr` → `/admin/committee`, `/admin/daily-qr` → `/admin/quests`, and top-level `/present` → `/admin/present`. Bare `/admin` redirects to `/admin/dashboard`. `app/admin/error.tsx` is the area's error boundary — it exists specifically to catch the `throw new Error('Unauthorized')` the `requireAdmin()` checks in the server actions raise, and renders it as a permission message in the admin light theme. Keep throwing that exact string if you add a guard.
...
- `app/(auth)/register` — multi-step: name/email/password → questionnaire (major, hobby, medical note, past achievements **required**; Instagram optional; gender optional) → pixel-avatar builder, previewed live with `<PixelAvatar>`. POSTs to `app/api/auth/register/route.ts`, which hashes the password, generates a `NSO-XXXXXXXX` `studentId`, builds the `avatarConfig` object, and inserts the `Student`, then auto signs in. The route has an explicit retry that strips `gender` and re-inserts if that column is missing, for partially-migrated databases. New students default to `isAdmin: false`, `role: 'student'`.
  - The form is **`noValidate` on purpose** and gates each step through `validateStep()` in the same file. Native `required` on a multi-step form that renders every step in one `<form>` fails invisibly: the browser refuses the submit, focuses a control that may be on a hidden step, and shows nothing — which users report as "the Next button does nothing" or "it just refreshes". Keep validation in JS, keep it mirroring the server checks in `app/api/auth/register/route.ts`, and keep the `document.getElementById(problem.field).focus()` call that points at the offending input.
...
**Group mascot art**: the 15 houses each have `public/images/group/<key>.png` (emblem, sliced by `scripts/extract-group-logos.mjs`) and `public/images/profile/<key>-bg.png` (the backdrop behind the avatar on `/profile`). The key is derived from `Group.name` by lowercasing and stripping non-letters — there is **no lookup table joining a group row to its art**, so a `Group` renamed to something outside the 15 silently falls back to no emblem and the neutral `avatar-bg.png`. Both `app/(game)/profile/page.tsx` and `app/(game)/leaderboard/page.tsx` separately normalize the one spelling mismatch (`nympth` → `nymph`); profile then has to map it *back* via `BG_FILENAME` because the backdrop shipped as `nympth-bg.png` while the emblem is `nymph.png`. Match an existing filename rather than adding a third spelling.
...
**Admin mutations**: server actions, each starting with a `requireAdmin()`-style session check (defined locally per file — there is no shared helper) and ending with `revalidatePath(...)`. They are split across **five** files — `app/admin/actions.ts` (groups, points, announcements, NPC/committee, clubs), `app/admin/quests/actions.ts` (quest CRUD, including the `availableFrom`/`availableUntil` window and a fallback that retries the write without those fields on an unmigrated DB), `app/admin/achievements/actions.ts`, `app/admin/timeline/actions.ts` (agenda rows only — the days themselves are code, see "Timeline"), and `app/admin/lunch/actions.ts`. Those `requireAdmin()` helpers **throw** rather than redirect, and `app/admin/error.tsx` is the boundary that catches it — it renders "You do not have permission" for the literal message `'Unauthorized'` and a generic message otherwise. It is the only error boundary in the repo, so an admin action that throws is handled while the same throw from a student page is not. Prefer adding admin writes here over new API routes. Note the timeline actions revalidate `/map/timeline` alongside `/info/timeline`; that path is now only a redirect, so the extra call is harmless but no longer meaningful.
...
**Fun-fact flow**: `/api/qr/generate` creates (or, with an `npcId`, regenerates for) an NPC and stores token + QR image on the row → student POSTs the token to `/api/qr/scan` → `lib/scan/npc.ts` guards, then `scan_npc` atomically rejects duplicates via `ScanLog`, creates the log row, and increments student `points`/`xp`/`funFactsCollected`, group `totalPoints`, and NPC `scanCount`. `NPC.maxScans` (NULL = unlimited) is enforced **in `lib/scan/npc.ts`, not in the RPC**: it rejects at the cap and flips `isActive = false` once the cap is reached. The student's own history is `GET /api/qr/recent` (their last 20 `ScanLog` rows joined to `NPC`, plus an all-time count), rendered by `app/(game)/scan/RecentScansPopup.tsx` — the only reader of that route.
...
**Committee vs. QR**: committee members are `NPC` rows — one table, and since the consolidation, one admin view. `/admin/committee` is the roster (grouped by division, matching `/info/committee`) *and* where you generate/regenerate/print QRs. `lib/divisions.ts` is the single source of truth for the six divisions (`isDivisionId`, `divisionName`, `DIVISIONS`). **Head-of-division ordering is not**: both `/api/committee` (student view) and `app/admin/committee/page.tsx` (admin view) sort heads to the top of their division with their own inline `/head|ketua/i` test against `NPC.role` — a duplicated stable sort, so changing the rule (or the wording admins type into `role`) means editing both files. Removing a member is a **soft delete** (`isActive = false`, via `deactivateCommitteeMember`) — it keeps their `ScanLog`/points history and hides them from the roster; `lib/scan/npc.ts` separately rejects scans against an inactive NPC.
...
**`.game-column` is the student app's layout contract.** Every in-game page's top-level wrapper div carries it: a centered column that steps 448px → 520px → 640px → 720px → 800px across breakpoints, so the pixel-art UI reads as a phone game in a window instead of stretching across a desktop. Thirteen files carry it: every `(game)/*` page **except `scan` and the `lunch/*` pages**, plus `Navbar` and `LunchShell` (which is what puts the lunch pages inside the column). `scan` is the genuine exception — it opts out with its own `max-w-lg` wrapper because the camera viewfinder wants a narrower frame. A new student page without `.game-column` will look wrong on any wide screen — and only on a wide screen, so it survives a phone-sized dev check.

**Group ("house") art is filename convention, not data.** A `Group` row carries `name`, `color`, `emblem`/`emblemUrl`; the mascot and house backdrop are looked up by slugifying `name` (`lowercase`, strip non-letters) against a hardcoded set of the fifteen mascots — `public/images/group/<key>.png` for the mascot close-up and `public/images/profile/<key>-bg.png` for the profile backdrop. Two traps: the design spells Nymph **"NYMPTH"**, so `'nympth'` is normalized to `nymph` on input *and* mapped back for the backdrop file, which actually shipped as `nympth-bg.png`; and there is **no shared helper** — the lookup is duplicated in `app/(game)/leaderboard/page.tsx` and `app/(game)/profile/page.tsx`, so a new group or a renamed asset means editing both. `components/ui/GroupEmblem.tsx` is the unrelated small-emblem component (uploaded `emblemUrl`, falling back to the emoji `emblem`).
