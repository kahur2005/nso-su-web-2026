...
// Maximum items shown in the player and record tabs.
const TOP_PLAYERS = 10;
const RECENT_RECORDS = 10;

// Styling properties for leaderboard ranks.
const RANK_STYLE = [
  { color: "#FFC073", shadow: "0px 2px 0px #ff5722", edge: "#ffc20e" }, // gold
  { color: "#747474", shadow: "0px 2px 0px #424242", edge: "#cfd4da" }, // silver
  { color: "#a15548", shadow: "0px 2px 0px #773b50", edge: "#b06a34" }, // bronze
];
const RANK_REST = { color: "#88684e", shadow: "none", edge: null };

// Generates CSS class and style for the podium trim.
...
// Resolves mascot image source by name.
...
// Standardizes Instagram profile URLs.
...
// Renders a pixel-art progress bar.
...
// Displays a mascot image or group emblem.
function GroupIcon({
  name,
  emblem,
  emblemUrl,
    ...
    <PageWrapper>
      ...
      <div className="relative">
        ...
        <div className="game-column pt-3 sm:pt-5 pb-6 sm:pb-8 md:pb-6">
          ...
          <div
            className="mb-2 flex items-center justify-end gap-1.5 px-2"
            data-tour="lb-live"
          >
            ...
            <span
              className="font-bytebounce text-[13px] text-[#86efac] tracking-wide"
              style={{ textShadow: "1px 1px 0 #14532d" }}
            >
              LIVE 30s
            </span>
          </div>
          ...
          {/* Current Leader */}
          {activeTab !== "record" &&
            (activeTab === "groups" ? leaderGroup : leaderStudent) && (
              <div
                className="relative mx-auto mb-4 w-[88%] max-w-[350px]"
                data-tour="lb-leader"
              >
                <div className="relative aspect-[337/249]">
                  ...
                  {/* Stars */}
                  <img
                    src="/images/leaderboard/star.png"
                    alt=""
                    aria-hidden
                  ...
                  <div className="absolute inset-x-0 top-[25%] flex h-[46%] items-center justify-center pt-2">
                    {activeTab === "groups" ? (
                      <GroupIcon
                        ...
                        className="h-[88%] w-[55%] translate-y-[6%]"
                      />
                    ) : (
                      <PixelAvatar
                        {...(() => {
                          const a = parseAvatarConfig(
                            leaderStudent.avatarConfig,
                          );
                          return {
                            skin: a.skin,
                  ...
                  <div className="absolute inset-x-[2%] top-[58%]">
                    <img
                      src="/images/leaderboard/banner-red.png"
                      alt=""
                      aria-hidden
          ...
          <p
            className="mb-1 text-center font-bytebounce text-[13px] text-[#e0b391]"
            style={{ textShadow: "1px 1px 0 #4e342e" }}
          >
            Updated {formatJakartaTime(lastUpdate)}
            <button
              onClick={fetchData}
              className="ml-2 text-[#7aff06] hover:brightness-125"
            >
              [refresh]
            </button>
          </p>
                  ...
                  {feed.slice(0, RECENT_RECORDS).map((ev) => (
                    ...
                    >
                      <span className="flex-shrink-0 text-lg">
                        {typeIcon[ev.questType] ?? "📋"}
                      </span>
                      <div className="min-w-0 flex-1">
                        <p className="truncate font-bytebounce text-[15px] leading-tight text-[#3e2723]">
                          {ev.studentName}
                        </p>
                        <p className="truncate font-bytebounce text-[12px] leading-tight text-[#5d4330]">
                          {ev.label}
                        </p>
                      </div>
                      <div className="flex-shrink-0 text-right">
                        <p className="font-bytebounce text-[15px] leading-none text-[#5d4330]">
                          +{ev.points} pts
                        </p>
                        <p className="mt-[2px] font-bytebounce text-[11px] leading-none text-[#a58962]">
                          {formatJakartaDate(ev.at, {
                            month: "short",
                            day: "numeric",
                          })}
                        </p>
