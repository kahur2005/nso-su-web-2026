...
export default function AdminPresenterPage() {
  ...
  useEffect(() => {
    if (
      ...
      !filteredNpcs.some((n) => n.id === selectedNpcId)
    ) {
  ...
  const fetchLiveQr = useCallback(async (npcId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/qr/live?npcId=${encodeURIComponent(npcId)}`,
      );
      ...
      // Keep existing QR code on network failure.
    } finally {
      setFetchingQr(false);
    }
  }, []);
  ...
  // Count down and rotate QR code automatically.
  useEffect(() => {
    ...
    return (
      <div className="p-8 max-w-md mx-auto text-center border border-slate-200 bg-white rounded-xl shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Only Committee Members, Group Leaders, or Admins can access the Live
          QR Presenter.
        </p>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
          ...
          >
            {filteredNpcs.length === 0 ? (
              <option value="" disabled>
                No committee members match search
              </option>
            ) : (
          ...
          <div className="mt-5 w-full max-w-[280px]">
            ...
            <div className="h-2 w-full rounded-full bg-slate-800 overflow-hidden border border-slate-700">
              <div
                ...
                style={{
                  width: `${(countdown / ROTATION_INTERVAL_SEC) * 100}%`,
                }}
              />
            </div>
          </div>
      ...
      <p className="text-center text-xs text-slate-500">
        🔒 Rolling QR presenter with short-lived tokens prevents off-site
        screenshot sharing.
      </p>
    </div>
  );
