...
export default function CartBar({
  ...
  return (
    <div className="fixed inset-x-0 bottom-20 z-40 px-2 md:bottom-4">
      ...
      >
        <span className="font-bytebounce text-fluid-md leading-none text-white">
          {count} item{count === 1 ? '' : 's'}
        </span>
        <span className="flex-1 text-right font-bytebounce text-fluid-lg leading-none text-[#ffd23f]">
          {formatRupiah(cartSubtotal(lines))}
        </span>
        <span className="font-bytebounce text-fluid-md uppercase leading-none text-white">
          View cart →
        </span>
      </Link>
    </div>
